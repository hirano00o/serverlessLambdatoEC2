...
DECRYPTED_WEBHOOK = boto3.client('kms').decrypt(CiphertextBlob=b64decode(ENCRYPTED_WEBHOOK))['Plaintext'].decode('utf-8')
DECRYPTED_TTOKEN = boto3.client('kms').decrypt(CiphertextBlob=b64decode(ENCRYPTED_TTOKEN))['Plaintext'].decode('utf-8')
DECRYPTED_BTOKEN = boto3.client('kms').decrypt(CiphertextBlob=b64decode(ENCRYPTED_BTOKEN))['Plaintext'].decode('utf-8')
WEBHOOK_URL = "https://hooks.slack.com/services/" + DECRYPTED_TTOKEN + "/" + DECRYPTED_BTOKEN + "/" + DECRYPTED_WEBHOOK
HISTRY_URL = 'https://slack.com/api/channels.history'
DELETE_URL = 'https://slack.com/api/chat.delete'
ENCRYPTED_LEGACY_TOKEN = os.environ['LEGACY_TOKEN']
DECRYPTED_LEGACY_TOKEN = boto3.client('kms').decrypt(CiphertextBlob=b64decode(ENCRYPTED_LEGACY_TOKEN))['Plaintext']
...
class Command:
    ...
    def start_command(self, ec2):
        ...
        try:
            ...
            return postSlack(self.param + " is started.\nstatus is " + str(status["ResponseMetadata"]["HTTPStatusCode"]))
